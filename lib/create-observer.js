const threshold = Array.from({ length: 1001 }, (_, idx) => idx * 0.001);

const createObserver = (rect) => {
  let callback = Function.prototype;
  const callbackWrapper = (arg) => {
    callback(arg);
  };
  ...
  return [new IntersectionObserver(callbackWrapper, config), setCallback];
};
