 ...
 * @returns {number}
 */
const normalizeTo1 = (number) => (Math.abs(1 - number) < 0.005 ? 1 : number);
  ...
  static #createObserver(ctx) {
    ...
    */
    ...
    if (recreationList.has('top')) {
      ...
      options.rootMargin = `-1px 0px ${-Math.min(vh - top - 2, vh - 1)}px 0px`;
    ...
    if (recreationList.has('left')) {
      ...
      options.rootMargin = `0px ${-Math.min(vw - left - 2, vw - 1)}px 0px -1px`;
